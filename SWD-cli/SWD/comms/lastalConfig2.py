...
#-a cost
gapExistCost=None
#Gap extension cost. A gap of size k costs: gapExistCost + (gapExtendCost × k).
#-b cost
gapExtendCost=None	
...
#This option allows use of "generalized affine gap costs" (SF Altschul 1998, Proteins 32(1):88-96). Here, a "gap" may consist of unaligned regions of both sequences. If these unaligned regions have sizes j and k, where j ≤ k, the cost is: a + b⋅(k-j) + c⋅j. If c ≥ a + 2b (the default), it reduces to standard affine gaps.
#-c Cost
afflineGap=None	
...
#Look for initial matches starting only at every STEP-th position in each query (positions 0, STEP, 2×STEP, etc). This makes lastal faster but less sensitive.
#-k Step
searchStep=None	
